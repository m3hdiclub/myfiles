...
chmod +x "$INSTALL_DIR/menu.sh" \
		 ...
		 "$INSTALL_DIR/telegrambot_freeserver.sh" \
		 "$INSTALL_DIR/telegrambot_cloudflaremanagerbot.sh" \
		 "$INSTALL_DIR/telegrambot_gcoremanagerbot.sh" \
...
echo -e "${BLUE}Creating restart xuibot command...${NC}"
cat > /usr/local/bin/restart << 'EOF'
#!/bin/bash
if [ "$1" = "xuibot" ]; then
    systemctl restart xuibot.service
fi
EOF
