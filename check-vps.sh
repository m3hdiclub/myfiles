...
cmd_to_pkg[wget]="wget"
cmd_to_pkg[jq]="jq"
cmd_to_pkg[host]="bind-utils dnsutils"
cmd_to_pkg[dig]="bind-utils dnsutils"

for tool in curl wget jq host dig; do
    if ! command -v $tool &> /dev/null; then
        missing_tools+=("$tool")
        for pkg in ${cmd_to_pkg[$tool]}; do
...
if [ ${#missing_tools[@]} -ne 0 ]; then
    ...
    if command -v apt-get &> /dev/null; then
        ...
        echo -e "${YELLOW}[TIP]${NC} On Debian/Ubuntu systems, run: apt-get install -y ${missing_packages[@]}"
        echo -e "${YELLOW}[TIP]${NC} On RHEL/CentOS systems, run: yum install -y ${missing_packages[@]}"
        exit 1
    fi
    ...
    if [ ${#still_missing[@]} -ne 0 ]; then
        echo -e "${RED}[ERROR]${NC} Failed to install: ${still_missing[*]}"
        echo -e "${YELLOW}[TIP]${NC} For host and dig commands, try installing 'dnsutils' (Debian/Ubuntu) or 'bind-utils' (RHEL/CentOS)"
        exit 1
    fi
fi
